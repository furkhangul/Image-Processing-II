...
"""
import numpy as np
from adodbapi.apibase import cvtInt
...
while True:
    ...
    if success:
        ...
        #Kontür ekleyecek olursak:
        (_,contures) = cv2.findContours(maske.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
        merkez = None

        if len(contures) > 0:
            #En büyük kontürü bul
            c  = max(contures, key = cv2.contourArea)
            #Dikdörtgene çevir
